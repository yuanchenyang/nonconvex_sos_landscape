...
function affine_rank_one_nonzero_const_probe(samples::Int = 20)
    canonical = Dict{Float64, NamedTuple}()
    for t in (-3.0, -2.0, -1.0, -0.5, 0.0, 0.5, 1.0, 2.0, 3.0)
        canonical[t] = image_rank_missing([
            quad_coeffs(a00 = t, a10 = 1.0),
            quad_coeffs(a20 = 1.0),
            quad_coeffs(a11 = 1.0),
            quad_coeffs(a02 = 1.0),
        ])
    ...
    for _ in 1:samples
        C = randn(3, 3)
        abs(det(C)) < 1e-3 && continue
        q2 = quad_coeffs(a20 = C[1, 1], a11 = C[1, 2], a02 = C[1, 3])
        q3 = quad_coeffs(a20 = C[2, 1], a11 = C[2, 2], a02 = C[2, 3])
        q4 = quad_coeffs(a20 = C[3, 1], a11 = C[3, 2], a02 = C[3, 3])
        data = image_rank_missing([quad_coeffs(a00 = 1.0, a10 = 1.0), q2, q3, q4])
    ...
    println("affine_rank_one_nonzero_const_probe = ", result)
    result
end

affine_rank_one_nonzero_const_probe()
