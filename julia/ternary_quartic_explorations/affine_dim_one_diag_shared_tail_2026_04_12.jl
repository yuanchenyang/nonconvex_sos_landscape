...
function affine_dim_one_diag_shared_tail_probe(; samples = 20)
    ...
    for _ in 1:samples
        ...
        push!(pure_tail,
            (; a, c, exact_affine_dim = exact_affine_dim(u_pure), image_rank_missing(u_pure)...))
        push!(mixed_tail,
            (; β, a, c, exact_affine_dim = exact_affine_dim(u_mixed), image_rank_missing(u_mixed)...))
    end
    ...
    println("affine_dim_one_diag_shared_tail_probe = ", result)
    result
end

affine_dim_one_diag_shared_tail_probe()
