...
3. **Lean formalization**: Translate the written proof into Lean files in `TernaryQuartic/TernaryQuarticProof/`. One logical unit per file.
4. **Verify**: Run `./TernaryQuartic/verify.sh`. Update `.tex` documents. Commit.
...
- **Statement (read-only)**: `TernaryQuartic/TernaryQuartic.lean`
- **Root proof file**: `TernaryQuartic/TernaryQuarticProof.lean` — must contain the final theorem declaration
- **Proof modules**: `TernaryQuartic/TernaryQuarticProof/` — all helper lemmas go here
- **Theorem**: `TernaryQuartic.ternaryQuartic_rankFour_no_spurious_socp`

**File-scope rules** (non-negotiable):

- Add new Lean files **only** inside `TernaryQuartic/TernaryQuarticProof/`. Add `import TernaryQuartic.TernaryQuarticProof.<Module>` to the root file as needed.
- You may modify `TernaryQuartic/TernaryQuarticProof.lean` (root import file).
- You may **not** modify `TernaryQuartic/TernaryQuartic.lean`, `scripts/verify_formalization.sh`, or `TernaryQuartic/verify.sh`.
...
**`blueprint.tex`** — Self-contained mathematical proof of the full theorem serving as the formalization blueprint. Each section maps to a `TernaryQuartic/TernaryQuarticProof/` file. State every lemma with a complete written proof. Mark which lemmas are formalized vs. pending. Write the proof in words here **before** formalizing in Lean.
...
1. `./TernaryQuartic/verify.sh` exits successfully (no `sorryAx`).
2. The theorem `TernaryQuartic.ternaryQuartic_rankFour_no_spurious_socp` is proved in `TernaryQuartic/TernaryQuarticProof.lean`.
