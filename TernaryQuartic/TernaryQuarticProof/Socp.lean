...
@[simp] theorem A_self_eq_sigma (u : RankFourVec) : A u u = sigma u := by
  simp [A, sigma, pow_two]

theorem focp_sigma_residual_eq_zero {B : DotForm} {p : Poly} {u : RankFourVec}
...
theorem objective_nonneg (p : Poly) (u : RankFourVec) : 0 ≤ objective B p u := by
  simpa [objective, LinearMap.BilinMap.toQuadraticMap_apply] using
    (Fact.out : B.toQuadraticMap.PosDef).nonneg (residual p u)

theorem objective_eq_zero_iff_residual_eq_zero {p : Poly} {u : RankFourVec} :
