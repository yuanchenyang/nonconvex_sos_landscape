...
/-
`TernaryQuartic/TernaryQuartic.lean` is the immutable statement file for the root
formalization. Exploration agents may read this file but should treat it as
read-only. The only intended write target is `TernaryQuartic/TernaryQuarticProof.lean`.
-/
